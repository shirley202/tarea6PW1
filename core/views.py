...
def pdf_to_html(request):
    if request.method == 'POST':
        pdf_ids = request.POST.getlist('selected_pdf_ids')
        if pdf_ids:
            pdf_instances = PDF.objects.filter(id__in=pdf_ids)
            html_contents = []

            for index, pdf_instance in enumerate(pdf_instances):
                identificacion = {
                ...
                html_content = render_to_string('pdf_to_html_template.html', {
                    'nombre_archivo': pdf_instance.nombre,
                    'identificacion': identificacion,
                    'secciones': secciones
                })

                # Añadir un marcador de página solo si no es el último PDF
                if index < len(pdf_instances) - 1:
                    html_content += '<div class="page-break"></div>'

                html_contents.append(html_content)

            return HttpResponse(''.join(html_contents))
        else:
...
def lista_programas(request):
    selected_carrera_id = request.GET.get('id')
    carreras = Carrera.objects.all()
    pdfs = PDF.objects.none()
    if selected_carrera_id:
        pdfs = PDF.objects.filter(carrera_id=selected_carrera_id).order_by('curso', 'semestre')
...
def imprimir_programas(request):
    if request.method == 'POST':
        ...
        pdfs_seleccionados = PDF.objects.filter(id__in=pdf_ids)

        # Pasar los datos de los PDFs seleccionados al template
        context = {
            'pdfs': pdfs_seleccionados
        }
        return render(request, 'pdf_to_html_template.html', context)

    return redirect('carrera_pdf_list')
